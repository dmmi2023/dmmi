...
from util.visualise import generate_labels


def train_one_epoch(model: torch.nn.Module, glip_booster: torch.nn.Module, criterion: torch.nn.Module,
    ...
    for samples, targets, glip_samples, glip_targets \
        in metric_logger.log_every(data_loader, print_freq, writer, epoch, header):

        # generate GT labels for subject perceptron
        with torch.no_grad():
            s_GT = []
            _, t, _, _, _ = glip_samples.tensors.shape
            glip_samples = glip_samples.to(device)
            
            # for each batch
            for (glip_sample, glip_target) in zip(glip_samples.tensors, glip_targets):
                glip_images = glip_sample.clone() 
                glip_captions = [glip_target["caption"] for i in range(t)]
                glip_output, scores, shapes, positive_map, pseudo_label, noun_chunks \
                    = glip_booster(1, glip_images, captions=glip_captions
                    )
                pseudo_label = generate_labels(images=glip_images, boxes=glip_output, targets=glip_target, \
                    positive_map=positive_map, pseudo_label=pseudo_label
                    )
                s_GT.append(pseudo_label.unsqueeze(0))     # list len = batch_size

        # DMMI inference
        b, t, c, h, w = samples.tensors.shape
        samples = samples.to(device)
        captions = [t["caption"] for t in targets]
        targets = utils.targets_to(targets, device) 

        outputs = model(samples, captions, targets) 
        loss_dict = criterion(outputs, targets, s_GT)
