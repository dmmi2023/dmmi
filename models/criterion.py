...
class SetCriterion(nn.Module):
    ...
    def __init__(self, num_classes, matcher, weight_dict, eos_coef, losses, focal_alpha=0.25):
        ...
        self.subject_perp = nn.CrossEntropyLoss()
    ...
    def forward(self, outputs, targets, pseudo_labels):
        ...
        """
        outputs_without_aux = {k: v for k, v in outputs.items() if k != 'aux_outputs' and k != 'subject'}
        ...
        if 'subject' in outputs and pseudo_labels is not None:
            subject_loss = 0
            for pseudo_label, output_subject in zip(pseudo_labels, outputs['subject']):
                len_label = min(pseudo_label.shape[-1], output_subject.shape[-1])
                pseudo_label = pseudo_label[:,:len_label]
                output_subject = output_subject.unsqueeze(0)
                output_subject = output_subject.permute(0, 2, 1)[:,:,:len_label]    # [1, 2, len_label]
                subject_loss += self.subject_perp(output_subject, pseudo_label)
            
            # try:
            #     subject_loss = self.subject_perp(outputs['subject'].permute(0,2,1), subject_label)
            # except:
            #     l1 = outputs['subject'].permute(0,2,1).shape[-1]
            #     l2 = subject_label.shape[-1]
            #     ooo = outputs['subject'].permute(0,2,1)
            #     if l2 > l1:
            #         subject_label = subject_label[:,:l1]
            #     else:
            #         ooo = ooo[:,:,:l2]
            #     subject_loss = loss(ooo, subject_label)

            losses.update({'subject': subject_loss})
            # output.backward()
