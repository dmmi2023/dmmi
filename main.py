...
from maskrcnn_benchmark.booster import build_glip_booster
...
def main(args):
    ...
    if args.use_glip:
        ...
        # glip booster
        glip_booster, cfg = build_glip_booster(args, utils.get_rank())
        glip_booster.to(device)
        if args.distributed:
            glip_booster = torch.nn.parallel.DistributedDataParallel(glip_booster, device_ids=[args.gpu])
            glip_booster = glip_booster.module

        checkpointer = DetectronCheckpointer(cfg, glip_booster, save_dir=cfg.OUTPUT_DIR)
        if args.booster_checkpoint:
            _ = checkpointer.load(args.booster_checkpoint, force=True)
    ...
    writer = SummaryWriter(f'runs/{args.tb_split}')
    start_time = time.time()
    for epoch in range(args.start_epoch, args.epochs):
        if args.distributed:
            sampler_train.set_epoch(epoch)
        train_stats = train_one_epoch(
            model, glip_booster, criterion, data_loader_train, optimizer, device, epoch, writer, 
            args.clip_max_norm)
        lr_scheduler.step()
        if args.output_dir:
